...
def cal_limit(or_eq):
    ...
    num_to = -1
    if or_eq[index_to + 3].isdigit() or (or_eq[index_to + 3] == '-' and or_eq[index_to + 4].isdigit()):
        num_to_start = index_to + 3
        num_to = int(or_eq[num_to_start:num_to_end])
    elif or_eq[index_to + 4].isalpha():
        num_to = oo
    elif (or_eq[index_to + 3] == '-' and or_eq[index_to + 5].isalpha()):
        num_to = -oo

    limit_str = or_eq[num_to_end+1:]
